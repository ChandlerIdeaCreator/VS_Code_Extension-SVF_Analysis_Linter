...
exports.SvfFlowTreeProvider = void 0;
exports.activate = activate;
exports.parseSVFOutput = parseSVFOutput;
exports.deactivate = deactivate;
exports.registerSvfTreeView = registerSvfTreeView;
...
function activate(context) {
    ...
    const runDisposable = vscode.commands.registerCommand('svf.runAnalysis', async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`SVF: 分析完成，共发现 ${issues.length} 个问题。`);
        }
        catch (err) {
 ...
 * 这样解析器的输出可以独立被单元测试。
 */
function parseSVFOutput(rawOutput) {
    const issues = [];
...
class SvfFlowTreeProvider {
    ...
    getTreeItem(element) {
        if (element.type === 'flow') {
            // 父节点：可折叠，默认展开
            const item = new vscode.TreeItem(element.flow.label, vscode.TreeItemCollapsibleState.Expanded);
    ...
    getChildren(element) {
        if (!element) {
            return this.flows.map(flow => ({ type: 'flow', flow }));
        }
        if (element.type === 'flow') {
            return element.flow.steps.map(step => ({ type: 'step', step, parent: element.flow }));
        }
        return [];
    }
}
exports.SvfFlowTreeProvider = SvfFlowTreeProvider;
/** 工具：根据传播步类型返回不同主题图标 */
 ...
 * 在 activate() 里调用一次即可。
 */
function registerSvfTreeView(context) {
    ...
    const gotoDisposable = vscode.commands.registerCommand('svf.gotoLine', async (line, filePath) => {
        try {
            ...
            editor.revealRange(new vscode.Range(safeLine, 0, safeLine, targetLineText.length), vscode.TextEditorRevealType.InCenter);
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
